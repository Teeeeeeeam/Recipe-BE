...
public class SingUpValidController {
    ...
    @Operation(summary = "로그인아이디 검증", description = "사용가능한 아이디인지 검증(대문자나 소문자(5~16)자리)")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "OK",
                    content = @Content(schema = @Schema(implementation = ControllerApiResponse.class),
                            examples = @ExampleObject(value = "{\"use_loginId\": \"[true] or [false].\"}"))),
            @ApiResponse(responseCode = "500", description = "INTERNAL SERVER ERROR",
                    content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
    })
    @PostMapping("/api/members/register/id/validate")
    public ResponseEntity<Map<String,Boolean>> LoginIdValid(@RequestBody LoginIdValidDto loginIdValidDto){
        try {
    ...
    public ResponseEntity<?> check(@Parameter(description = "인증 번호")@RequestParam("code")String UserCode){
        try {
